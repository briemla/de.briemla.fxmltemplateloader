...
public class FxmlTemplateLoaderIntegrationTest {
	...
	/**
	 * When there are no exceptions, the image has been successfully loaded and
	 * the mechanism works.
	 *
	 * @throws Exception
	 *             throws {@link InterruptedException} when FXApplication thread
	 *             could not be started and an {@link IOException} when FXML
	...
	public void loadReferencedElementInstantiateTemplateTwice() throws Exception {
		ITemplate template = loadTemplate("VBoxWithReferencedElement");
	...
	public void loadVBoxWithFxControllerButAlreadySetControllerOnLoader() throws Exception {
		...
		loader.doLoad(fromResource("VBoxWithFxController"));
	}

	@Test
	public void loadVBoxWithFxController() throws Exception {
		ITemplate template = loadTemplate("VBoxWithFxController");
		template.create();
	...
	public void initilizeController() throws Exception {
		ITemplate template = loadTemplate("VBoxWithInitializableFxController");
		template.create();
	...
	public void initializeControllerWithResources() throws Exception {
		ITemplate template = loadTemplate("VBoxWithResourceTakingFxController");
		template.create();
	...
	public void loadVBoxWithNestedFxController() throws Exception {
		...
		loader.doLoad(fromResource("VBoxWithNestedFxController"));
	}
	...
	private static <T> T load(String fileName) throws IOException {
		return FxmlTemplateLoader.load(fromResource(fileName));
	}
	...
	private static <T> T loadWithController(String fileName, Object controller) throws IOException {
		...
		return fxmlTemplateLoader.doLoad(fromResource(fileName));
	}

	private static <T> T loadWithRoot(String fileName, Object fxRoot) throws IOException, InstantiationException,
			IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		ITemplate template = loadTemplate(fileName);
		template.setRoot(fxRoot);
		return template.create();
	}

	private static ITemplate loadTemplate(String fileName) throws IOException {
		return FxmlTemplateLoader.loadTemplate(fromResource(fileName));
	}
